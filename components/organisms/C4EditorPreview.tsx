...
export function C4EditorPreview() {
    ...
    const [leftPaneMode, setLeftPaneMode] = useState<"editor" | "chat">(
        "editor",
    );
    ...
    const saveElement = () => {
        ...
        if (parsedStructure.aliases.has(alias)) {
            setFormError(
                "Такой alias уже существует. Укажите уникальный alias.",
            );
            return;
        }
    ...
    const sendAiRequest = async () => {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                throw new Error(
                    failed.message ||
                        `AI запрос завершился ошибкой (${response.status})`,
                );
            }
                ...
                <div className="space-y-3 md:space-y-4">
                    ...
                    <label className="space-y-1 text-xs text-(--color-foreground-dim)">
                        ...
                        <InputSmall
                            ...
                            onChange={(event) =>
                                setNewElementAlias(event.target.value)
                            }
                    ...
                    <label className="space-y-1 text-xs text-(--color-foreground-dim)">
                        ...
                        <InputSmall
                            ...
                            onChange={(event) =>
                                setNewElementLabel(event.target.value)
                            }
                ...
                <div className="space-y-3 md:space-y-4">
                    <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
                        <label className="space-y-1 text-xs text-(--color-foreground-dim)">
                            ...
                            <Select
                                ...
                                options={c4RelationshipTypeOptions.map(
                                    (item) => ({
                                        value: item.value,
                                        label: item.label,
                                    }),
                    ...
                    <label className="space-y-1 text-xs text-(--color-foreground-dim)">
                        ...
                        <InputSmall
                            ...
                            onChange={(event) =>
                                setNewRelationTechnology(event.target.value)
                            }
